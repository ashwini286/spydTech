...
const Slider = () => {
  ...
  const slideData = [
    ...
    {
      icons: <RiPagesLine className='text-blue-500'/>,
    ...
    {
      icons: <FaRegLightbulb className='text-yellow-500'/>,
    ...
    {
      icons: <FaCode className='text-cyan-400' />,
    ...
    {
      icons: <FaRegThumbsUp className='text-green-400' />,
    ...
    {
      icons: <GrOptimize className='text-blue-600'/>,
      ...
      <div className="grid lg:grid-cols-6 gap-4 mb-12 md:grid-cols-2 grid-cols-1">
        {slideData.map((slide, index) => (
          <div key={index} className="flex flex-col items-center space-y-2 relative ">
            <button className={`p-10 rounded-full bg-gray-300 text-4xl hover:bg-light-blue-900 ${index === visibleImageIndex ? 'active' : ''}`} onClick={() => scrollToImage(index)}>
              {slide.icons}
            </button>
            {index === visibleImageIndex && (
              <div className="active-indicator"></div>
            )}
            <p className="text-gray-700 text-center hover:text-blue-800 font-extrabold cursor-pointer">{slide.name}</p>
          </div>
        ))}
      </div>
      <div className="flex lg:flex-row justify-center items-center swiper flex-col">

   <div className='w-[50%] swiper-slide'>
          {slideData.map((slide, index) => (
            <div key={index} className={`description-wrapper mb-4 ${index === visibleImageIndex ? 'visible' : 'hidden'}`}>
              <p className="text-gray-700">{slide.name}</p>
              <p className="text-gray-700">{slide.description}</p>
            </div>
          ))}
        </div>
        <div className='w-[50%] swiper-slide' data-aos="fade-left">
          {slideData.map((slide, index) => (
            <div key={index} ref={imageRefs[index]} className={`image-wrapper mb-4 ${index === visibleImageIndex ? 'visible' : 'hidden'}`}>
              <img
                src={slide.image}
                alt={`Image ${index + 1}`}
